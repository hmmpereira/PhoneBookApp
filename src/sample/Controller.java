...
public class Controller {
        ...
        if(result.isPresent() && result.get() == ButtonType.OK) {
            ...
            Task<ObservableList<Contact>> task = new Task<ObservableList<Contact>>() {
                ...
                protected ObservableList<Contact> call() throws Exception {
                    return FXCollections.observableArrayList(contactController.searchContact());
                }
            };
            contactsTable.itemsProperty().bind(task.valueProperty());
    ...
    public void deleteContact() {
        ...
        alert.setContentText("Are you sure you want to delete the selected contact: " +
                selectedContact.getName() + " " + selectedContact.getAddress());

        Optional<ButtonType> result = alert.showAndWait();
        if(result.isPresent() && result.get() == ButtonType.OK) {
            Datasource.getInstance().deleteContacts(selectedContact.getName(), selectedContact.getAddress(), selectedContact.getPhoneNumber());
            listContacts();
        }
    }
}
...
class GetAllContactsTask extends Task {
    ...
    public ObservableList<Contact> call()  {
        return FXCollections.observableArrayList
                (Datasource.getInstance().queryEveryContact(Datasource.ORDER_BY_ASC));
    }
}
